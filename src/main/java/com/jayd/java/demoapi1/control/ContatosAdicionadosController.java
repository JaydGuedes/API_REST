...
public class ContatosAdicionadosController {
	...
	@DeleteMapping(path = "/{contId}", produces = "application/json")
	public String delete(@PathVariable Long id) {
		if (ContatosAdicionadosRepository.existsById(id)) {
			ContatosAdicionadosRepository.deleteById(id);
			return "{ \"status\" : \"deleted\" }";
		}
		return "{ \"status\" : \"error\" }";
	}

	@PutMapping(path = "/{contId}")
	public ContatosAdicionados put(@PathVariable Long id, @RequestBody ContatosAdicionados contatosAdicionados) {
		return null;
	}
